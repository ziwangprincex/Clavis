...
export function GitSection({ root, onRefresh }: GitSectionProps) {
  ...
  return <div className={styles.root}>
    <div className={styles.head}><span>{status?.isRepository ? (status.branch ?? (status.detached ? 'detached' : 'repository')) : t("no repository")}</span><span>{status && `${status.ahead ? `?${status.ahead} ` : ''}${status.behind ? `?${status.behind}` : ''}`}</span><button type="button" disabled={loading} onClick={onRefresh}>?</button></div>
    {error && <div className={styles.error}>{error}</div>}
    {status?.isRepository && <ul className={styles.files}>{status.files.map(file => <li key={file.path} className={selected === file.path ? styles.active : ''} onClick={() => void useGitStore.getState().selectFile(root, file.path)}><span>{file.untracked ? '?' : `${file.indexStatus}${file.worktreeStatus}`}</span><span>{file.path}</span><button type="button" onClick={event => { event.stopPropagation(); void (file.untracked || file.worktreeStatus !== ' ' ? useGitStore.getState().stage(root, file.path) : useGitStore.getState().unstage(root, file.path)); }}>{file.untracked || file.worktreeStatus !== ' ' ? t("Stage") : t("Unstage")}</button></li>)}</ul>}
    ...
    {status?.isRepository && <div className={styles.commit}><input value={message} maxLength={200} onChange={event => setMessage(event.target.value.replace(/[\r\n]/g, ''))} placeholder={t("Local commit message...")} /><button type="button" disabled={committing || !message.trim()} onClick={() => void createCommit()}>{committing ? t("Committing...") : t("Commit staged")}</button><small>{t("Local only - no push - hooks skipped")}</small></div>}
