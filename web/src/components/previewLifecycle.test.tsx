...
vi.mock('../api/tauri', () => ({ hasTauri: () => true, ipc: { compileTypst: vi.fn() } }));
...
beforeEach(() => {
  vi.useFakeTimers();
  vi.clearAllMocks();
  vi.mocked(ipc.compileTypst).mockResolvedValue({ ok: true, svg: '<svg>latest</svg>' });
...
describe('preview lifecycle without a browser', () => {
  ...
  it('serializes Typst updates and never displays stale output', async () => {
    let resolve!: (result: { ok: boolean; svg: string }) => void;
    ...
    await act(async () => { resolve({ ok: true, svg: '<svg>stale</svg>' }); });
    expect(ipc.compileTypst).toHaveBeenCalledTimes(2);
    expect(ipc.compileTypst).toHaveBeenLastCalledWith('third', '/p/main.typ');
  ...
  it('ignores an in-flight result after hide and refreshes unchanged Typst on reopen', async () => {
    ...
    await advance();
    act(() => tree!.update(<PreviewPane visible={false} />));
    await act(async () => { resolve({ ok: true, svg: '<svg>hidden</svg>' }); });
