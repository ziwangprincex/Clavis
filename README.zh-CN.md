...
Git 侧边栏显示仓库/分支状态、ahead/behind 计数、改动过或未跟踪的文件、最近的提交，
以及单个文件的差异。词级的文本视图会高亮增删；LaTeX 模式在比较前会忽略注释、空白和
常见的纯排版命令。

写操作有意做得很窄：可以对已经列为改动的文件 **Stage** / **Unstage**，然后创建一个
需要确认的**本地**提交。提交要求单行、非空、不超过 200 字符的消息，并且至少有一处
已暂存的改动；执行时带 `--no-gpg-sign` 和一个空的 `core.hooksPath`，所以仓库里的
hook 不会运行。确认框会先把完整的提交消息显示出来。

在 Clavis 里始终无法 push、fetch、pull、checkout、reset、恢复工作区、rebase、merge、
切换分支或联系远端。每次 Git 调用都是直接传 argv、不经过 shell，有 5 秒上限，stdin
为空；配置了 Git clean filter 的文件（比如 LFS）会被拒绝，而不是被暂存。
