/* Math symbol palette data.
 *
 * Each symbol has:
 *   c   — preview character (Unicode)
 *   l   — LaTeX command (without leading backslash; we'll add it on insert)
 *   t   — Typst expression (omit if same as LaTeX with $...$ wrapping)
 *
 * Inserts replace any selection at cursor. In LaTeX/Markdown editor inserts
 * `\command`. In Typst editor inserts the typst form (`alpha`, `int`, etc).
 */
 ...
 *
 *   - LaTeX / Markdown: returns "\\<cmd>" verbatim. If cmd contains "{}", caret
 *     should land between the braces; we represent that with the marker "$|".
 *   - Typst: returns the typst form. Wrapped in `$...$` only if the user is NOT
 *     already inside math mode — but we don't try to detect; user is responsible.
 */
export function symbolInsertText(sym, lang) {
  if (lang === 'typst') {
    return sym.t || sym.l;
  }
  // LaTeX / Markdown
